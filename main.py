...
app = FastAPI(title="Tomasz Trading Journal v2.5 — Full Screenshot View")
...
@app.get("/health")
def health():
    return {"ok": True, "version": "2.5", "entry_types": ["taken", "missed", "summary"], "trade_numbers": True, "r_metrics": True}
...
async def analytics(
    ...
    r = await sb_request(
        ...
        "/rest/v1/trades",
        params={
            "select": "id,trade_time,instrument,side,setup,pnl,tags,rating,entry_type,trade_number,entry,exit,risk_points,result_points",
    ...
    def row_points(row):
        return calc_result_points(
            row.get("side"),
            row.get("entry"),
            row.get("exit"),
            row.get("result_points"),
        )
    ...
    def outcome_value(row):
        """Prefer points for win/loss classification; fall back to PnL for old entries."""
        pts = row_points(row)
    ...
    def group_by(field: str):
        ...
        # R is the primary ranking. If R is not yet populated, fall back to points/PnL.
        return sorted(
            result,
            key=lambda x: (
                x["total_r"] if x["r_trades"] else -10**9,
                x["points"] if x["point_trades"] else x["pnl"],
                x["trades"],
            ),
            reverse=True,
        )
...
async def create_trade(
    ...
    parsed_entry = optional_float(entry)
    parsed_exit = optional_float(exit)
    parsed_result_points = optional_float(result_points)
    parsed_risk_points = optional_float(risk_points)
    if parsed_result_points is None:
        parsed_result_points = calc_result_points(side, parsed_entry, parsed_exit)
    if parsed_risk_points is not None and parsed_risk_points <= 0:
        parsed_risk_points = None
    if normalized_type == "summary":
        parsed_result_points = None
        parsed_risk_points = None
...
<title>Trading Journal v2.4</title>
<style>
...
<header class="header">
 <div class="header-row">
  <div><div class="brand">Trading Journal v2</div><div class="sub">Feed · R-multiple · punkty · PnL · setupy · każde urządzenie</div></div><div class="spacer"></div>
...
<dialog id="dlg">
 ...
 <div class="quick-grid">
  ...
  <div id="sideField"><label>Kierunek</label><select id="side" onchange="updateRPreview()"><option>LONG</option><option>SHORT</option></select></div>
  <div id="pnlField"><label id="pnlLabel">PnL (pomocniczo)</label><input id="pnl" type="number" step="any" placeholder="np. 250 albo -120"></div>
  <div id="resultPointsField"><label>Wynik w pkt (+ zysk / − strata)</label><input id="result_points" type="number" step="0.25" placeholder="np. 40 albo -18" oninput="updateRPreview()"></div>
  <div id="riskPointsField"><label>Ryzyko 1R w pkt</label><input id="risk_points" type="number" step="0.25" min="0" placeholder="np. 20" oninput="updateRPreview()"></div>
  <div id="rPreview" class="r-preview"><strong>R: —</strong><br><span>Wpisz wynik w pkt oraz początkowe ryzyko 1R.</span></div>
  ...
  <details class="details"><summary>Więcej szczegółów</summary><div class="form-grid">
   ...
   <div id="entryField"><label>Entry</label><input id="entry" type="number" step="any" oninput="updateRPreview()"></div><div id="exitField"><label>Exit</label><input id="exit" type="number" step="any" oninput="updateRPreview()"></div>
...
function updateRPreview(){
 ...
 if(type==='summary'){$('rPreview').style.display='none';return}
 $('rPreview').style.display='block';
 let pts=$('result_points').value.trim()===''?null:Number($('result_points').value);
 if(pts==null){
   const e=Number($('entry').value),x=Number($('exit').value);
   if(Number.isFinite(e)&&Number.isFinite(x)&&$('entry').value!==''&&$('exit').value!=='')pts=$('side').value==='LONG'?x-e:e-x;
 }
 const risk=Number($('risk_points').value),rv=pts!=null&&Number.isFinite(risk)&&risk>0?pts/risk:null;
 $('rPreview').innerHTML=`<strong class="${rv>0?'pos':rv<0?'neg':''}">R: ${rv==null?'—':signed(rv,2)+'R'}</strong><br><span>${pts==null?'Wynik pkt: —':'Wynik pkt: '+signed(pts,2)} · ${Number.isFinite(risk)&&risk>0?'1R = '+risk.toFixed(2)+' pkt':'Ryzyko 1R: —'}</span>`;
}
function syncEntryTypeUI(){
 const type=$('entry_type').value||'taken';
 const isTaken=type==='taken',isSummary=type==='summary';
 $('tradeNumberField').style.display=isTaken?'block':'none';
 ['sideField','pnlField','resultPointsField','riskPointsField','setupField','qtyField','entryField','exitField','ratingField'].forEach(id=>$(id).style.display=isSummary?'none':'block');
 $('pnlLabel').textContent=type==='missed'?'Hipotetyczny PnL (nie liczy się)':'PnL (pomocniczo)';
...
async function loadAnalytics(){const r=await fetch(`/api/analytics/${encodeURIComponent(key)}?period=${currentPeriod}`,{cache:'no-store'}),d=await r.json(),s=d.summary;
 ...
 $('sTotalR').textContent=s.r_trades?signed(s.total_r,2)+'R':'—';
 $('sTotalR').className='v '+(s.total_r>0?'pos':s.total_r<0?'neg':'');
 $('sAvgR').textContent=s.avg_r==null?'—':signed(s.avg_r,2)+'R';
 $('sAvgR').className='v '+(s.avg_r>0?'pos':s.avg_r<0?'neg':'');
 $('sPoints').textContent=s.point_trades?signed(s.net_points,1):'—';
 $('sPoints').className='v '+(s.net_points>0?'pos':s.net_points<0?'neg':'');
 $('sPnl').textContent=money(s.total_pnl).replace('+','');
 $('sPnl').className='v '+(s.total_pnl>0?'pos':s.total_pnl<0?'neg':'');
 $('sWin').textContent=Math.round(s.win_rate)+'%';
 $('miniWins').textContent=s.wins;
 $('miniLosses').textContent=s.losses;
 $('miniWonPts').textContent=s.point_trades?signed(s.won_points,1):'—';
 $('miniLostPts').textContent=s.point_trades?'-'+Number(s.lost_points||0).toFixed(1):'—';
...
async function loadFeed(){
 ...
 $('feed').innerHTML=d.items.map(t=>{
   ...
   const pts=pointsOf(t),rv=rOf(t),risk=t.risk_points!=null?Number(t.risk_points):null;
   const resultHead=type==='taken'
     ...
     :(type==='missed'?`<div class="trade-result"><div class="r-main ${rv>0?'pos':rv<0?'neg':''}">Hip. ${rv==null?'— R':signed(rv,2)+'R'}</div><div class="pts-sub">${pts==null?'— pkt':signed(pts,1)+' pkt'}</div><div class="cash-sub">NIE LICZY SIĘ</div></div>`:'<div class="nonstat">NIE LICZY SIĘ DO WYNIKÓW</div>');
   const tradeMeta=type==='summary'?'':`${risk!=null&&Number.isFinite(risk)?`<span>1R = ${risk.toFixed(1)} pkt</span>`:''}${t.entry!=null?`<span>Entry ${esc(t.entry)}</span>`:''}${t.exit!=null?`<span>Exit ${esc(t.exit)}</span>`:''}${t.qty!=null?`<span>Qty ${esc(t.qty)}</span>`:''}${t.rating?`<span>${'★'.repeat(Number(t.rating))}</span>`:''}`;
   return `<article class="card"><div class="card-head">${number}${symbol}${typeBadge}${sideBadge}${setupBadge}${resultHead}</div><div class="card-body"><div>${t.screenshot_url?`<img class="shot" src="${t.screenshot_url}" loading="lazy" onclick="openLightbox(this.src)" title="Kliknij, aby powiększyć">`:`<div class="shot" style="display:grid;place-items:center;color:var(--muted)">Brak screenshotu</div>`}</div><div><div style="font-size:12px;color:var(--muted)">${esc(date)}</div><div class="meta">${tradeMeta}${t.tags?`<span>${esc(t.tags)}</span>`:''}</div><div class="note">${esc(t.notes||'')}</div>${t.lesson?`<div class="lesson"><b>Wniosek:</b><div class="note">${esc(t.lesson)}</div></div>`:''}<div style="margin-top:13px"><button class="btn" onclick='editTrade(${JSON.stringify(t).replaceAll("'","&#39;")})'>Edytuj</button></div></div></div></article>`
 }).join('')
}
...
function openNew(){editingId=null;$('entry_type').value='taken';$('trade_number').value='';$('instrument').value='MNQ';$('side').value='LONG';$('pnl').value='';$('result_points').value='';$('risk_points').value='';$('setup').value='';$('notes').value='';['entry','exit','qty','rating','tags','lesson'].forEach(id=>$(id).value='');$('trade_time').value=nowLocal();$('screenshot').value='';$('preview').style.display='none';$('preview').src='';$('deleteBtn').style.display='none';$('dlgTitle').textContent='Dodaj trade';syncEntryTypeUI();dlg.showModal()}
window.editTrade=t=>{editingId=t.id;$('entry_type').value=entryTypeOf(t);$('trade_number').value=t.trade_number??'';$('instrument').value=t.instrument||'MNQ';$('side').value=t.side||'LONG';$('pnl').value=t.pnl??'';$('result_points').value=t.result_points??'';$('risk_points').value=t.risk_points??'';$('setup').value=t.setup||'';$('notes').value=t.notes||'';$('trade_time').value=localInputFromIso(t.trade_time);['entry','exit','qty','rating','tags','lesson'].forEach(id=>$(id).value=t[id]??'');$('screenshot').value='';const p=$('preview');if(t.screenshot_url){p.src=t.screenshot_url;p.style.display='block'}else{p.src='';p.style.display='none'}$('deleteBtn').style.display='inline-block';$('dlgTitle').textContent='Edytuj wpis';syncEntryTypeUI();dlg.showModal()}
